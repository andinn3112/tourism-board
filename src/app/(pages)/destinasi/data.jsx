...
export default function Data({ data }) {
   return (
      ...
      >
         {data?.length > 0 ? (
            data?.map((item, i) => <DestinationList data={item} key={i} />)
         ) : (
            <EmptyData title="Data destinasi tidak tersedia" />
         )}
      </div>
   );
}
   ...
   return (
      <div className="flex flex-col bg-card rounded-lg shadow-sm hover:shadow-md transition-shadow duration-300 overflow-hidden border border-card">
         ...
         >
            {data.images?.[0] ? (
               <Image
                  src={data.images[0]?.url || "images/no-preview.jpg"}
                  alt={data.nama}
                  fill
                  loading="eager"
